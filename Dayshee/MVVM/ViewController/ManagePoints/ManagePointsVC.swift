...
class ManagePointsVC: UIViewController {

    private let tableView: UITableView = UITableView(frame: .zero, style: .grouped)
    @IBOutlet weak var vTotalPoint: UIView!
...
extension ManagePointsVC {
    private func visualize() {
        ...
        tableView.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(vTotalPoint.snp.bottom)
        }
        tableView.delegate = self
        tableView.register(ManagePointCell.nib, forCellReuseIdentifier: ManagePointCell.identifier)
    }
    private func setupRX() {
        Observable.just([1])
            .bind(to: tableView.rx.items(cellIdentifier: ManagePointCell.identifier, cellType: ManagePointCell.self)) {(row, element, cell) in
...
extension ManagePointsVC: UITableViewDelegate {

...
class NewPasswordVC: UIViewController {
    ...
    private let viewModel: NewPasswordVM = NewPasswordVM()
    var delegate: InputCodeDelegate?
...
extension NewPasswordVC {
    ...
    private func setupRX() {
        ...
        self.btNext.rx.tap.bind(onNext: weakify({ (wSelf) in
            guard let pw = wSelf.tfNewPw.text,
                  pw != "",
                  let pwCF = wSelf.tfConfirmPw.text,
                  pwCF != "" else {
                wSelf.showAlert(title: nil, message: "Mật khẩu không được để trống")
                return
            }
            guard pw == pwCF else {
                wSelf.showAlert(title: nil, message: "Mật khẩu chưa khớp")
                return
            }
            let p: [String : Any] = [ "current_password": pw,
                                      "new_password": pwCF
            ]
            wSelf.viewModel.newPass(p: p)
        })).disposed(by: disposeBag)
        
        self.viewModel.$user.asObservable().bind { _ in
            let me = self
